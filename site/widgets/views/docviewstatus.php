...
<div id="doc-actions" class="rounded-lg border bg-card text-card-foreground shadow-sm">
    <div class="flex flex-col space-y-1.5 p-6">
        <h3 class="font-semibold tracking-tight text-base"><?= DictionaryService::getWord('status', $user->lang) ?></h3>
    </div>
    <div id="doc-status-block" class="p-6 pt-0 space-y-2">
        <p><?= DictionaryService::getWord('currentStatus', $user->lang) ?>: <strong><?= $document->getStatusName($user->lang) ?></strong></p>
        <button id="doc-change-status" data-doc-id="<?= $document->id ?>" class="inline-flex items-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50    border border-input bg-background hover:bg-accent hover:text-accent-foreground h-10 px-4 py-2 w-full justify-start">
