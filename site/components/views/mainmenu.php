...
<aside class="fixed left-0 top-0 z-40 h-screen bg-sidebar border-r border-sidebar-border transition-all duration-300 w-64">
    ...
    <nav class="flex flex-col gap-1 p-3 mt-2">
        ...
        ?>
            <a aria-current="page" class="menuitem flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground <?= $style ?>" href="<?= $value['url'] ?>">
                <?= $value['picture'] ?>
                <span><?= DictionaryService::getWord($name, $user->lang) ?></span>
            </a>
        <?php
        }
        ?>
