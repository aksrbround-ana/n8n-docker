...
$(document).on('click', 'tr.task-row', function (e) {
  ...
  let sortingInputs = $('div.all-filter-box input.sorting[value!=none]');
  let sorting = [];
  for (let i = 0; i < sortingInputs.length; i++) {
    let field = $(sortingInputs[i]).data('field');
    let value = $(sortingInputs[i]).val();
    if (value != 'none') {
      let sortObj = {
        field: field,
        value: value
...
$(document).on('click', 'button.sorting', function (e) {
  ...
  const page = $('div.pagination button.task-page.active').data('page');
  getTaskList(page);
});
